...
def _as_tensor_on_cuda(x) -> torch.Tensor:
    ...
    if isinstance(x, (list, tuple)):
        try:
            ...
            return torch.tensor([float(v) for v in x])
    if isinstance(x, (int, float)):
        return torch.tensor(x, dtype=torch.float32)
    return None

def _accumulate_metrics_summed_and_weight(eval_result, accum_sum, accum_cnt):
    for k, v in eval_result.items():
        if k.startswith("per_") or k.endswith("_per") or k in ("items", "paths"):
            continue
        t = _as_tensor_on_cuda(v)
        if t is None:
            continue
        t = t.float().to("cuda", non_blocking=True)
        if t.numel() == 0:
            continue
        if t.numel() == 1:
            val_sum = t.reshape(-1)[0]
            cnt = torch.tensor(1.0, device=t.device)
        else:
            val_sum = t.mean().reshape(1) * t.numel()
            val_sum = val_sum[0]
            cnt = torch.tensor(float(t.numel()), device=t.device)
        accum_sum[k] = accum_sum.get(k, torch.tensor(0.0, device=t.device)) + val_sum
        accum_cnt[k] = accum_cnt.get(k, torch.tensor(0.0, device=t.device)) + cnt

def _distributed_mean_reduce(accum_sum, accum_cnt):
    world = distributed.get_world_size()
    if world > 1:
        for d in (accum_sum, accum_cnt):
            for k in d:
                torch.distributed.all_reduce(d[k], op=torch.distributed.ReduceOp.SUM)
    out = {}
    for k in accum_sum:
        denom = torch.clamp(accum_cnt[k], min=1.0)
        out[k] = (accum_sum[k] / denom).item()
    return out
...
class ImaginaireTrainer:
    ...
    def train(
        ...
        if self.config.trainer.max_iter == 0:
            iteration = 0
        grad_accum_iter = 0
        ...
        with (
            ...
            while (self.config.trainer.max_iter != 0):
    ...
    def validate(self, model: ImaginaireModel, dataloader_val: torch.utils.data.DataLoader, iteration: int = 0) -> None:
        ...
        with model.pipe.ema_scope(context="Validation", is_cpu=False):
            for val_iter, data_batch in tqdm(enumerate(dataloader_val)):
                ...
                output_batch['path_local'] = self.config.job.path_local
                eval_result, samples_name = model.on_validation_step_end(model, data_batch, output_batch, loss, iteration=iteration)
                # self.callbacks.on_validation_step_end(model, data_batch, output_batch, loss, iteration=iteration)
                if isinstance(eval_result, dict):
                    pretty_step = " | ".join([f"{k}: {v:.6f}" for k, v in sorted(eval_result.items())])
                    log.info(f"[Val step {val_iter} @ {samples_name}] {pretty_step} | ")
                    _accumulate_metrics_summed_and_weight(eval_result, accum_sum, accum_cnt)

        metrics_mean = _distributed_mean_reduce(accum_sum, accum_cnt)
        if distributed.is_rank0():
            if len(metrics_mean) == 0:
                log.warning("No validation metrics were aggregated.")
            else:
                pretty = " | ".join([f"{k}: {v:.6f}" for k, v in sorted(metrics_mean.items())])
                log.success(f"[Val@iter {iteration}] {pretty}")
                try:
                    path_local = getattr(self.config.job, "path_local", None)
                    if path_local:
                        out_path = os.path.join(path_local, f"val_metrics_iter{iteration}.json")
                        os.makedirs(os.path.dirname(out_path), exist_ok=True)
                        with open(out_path, "w") as f:
                            json.dump({"iteration": iteration, "metrics": metrics_mean}, f, indent=2)
                        log.info(f"Validation metrics saved to {out_path}")
                except Exception as e:
                    log.warning(f"Failed to save validation metrics: {e}")
