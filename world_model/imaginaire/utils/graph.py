...
def create_cuda_graph(
    ...
    k_mem: torch.Tensor | None = None,    # [B, Lm, D]
    v_mem: torch.Tensor | None = None,    # [B, Lm, D]
) -> str:
    ...
    shapes_key = "_".join(
        str(s) for tensor in ([x] + [t for t in [affline_emb_B_D, crossattn_emb] if t is not None]
                              + [t for t in [rope_emb_L_1_1_D, adaln_lora_B_3D, extra_per_block_pos_emb, k_mem, v_mem]
                                 if t is not None])
        ...
        for block in blocks:
            ...
            for arg in real_args:
                if arg.dtype == torch.int64:
                    dummy = torch.randint(int(arg.min().item()), int(arg.max().item()) + 1, arg.shape, device=arg.device, dtype=arg.dtype)
                else:
                    dummy = torch.randn(arg.shape, device=arg.device, dtype=arg.dtype)
                dummy.requires_grad_(arg.requires_grad)
                args.append(dummy)

            kwargs = {}
            for name, kwarg in real_kwargs.items():
                if kwarg.dtype == torch.int64:
                    dummy = torch.randint(int(kwarg.min().item()), int(kwarg.max().item()) + 1, kwarg.shape, device=kwarg.device, dtype=kwarg.dtype)
                else:
                    dummy = torch.randn(kwarg.shape, device=kwarg.device, dtype=kwarg.dtype)
                dummy.requires_grad_(kwarg.requires_grad)
                kwargs[name] = dummy
