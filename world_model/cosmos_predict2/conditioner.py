...
@dataclass(frozen=True)
class ActionBlackCondition(VideoCondition):
    action: torch.Tensor | None = None
    blacks: torch.Tensor | None = None

@dataclass(frozen=True)
class ActionBlackHistoryCondition(VideoCondition):
    action: torch.Tensor | None = None
    blacks: torch.Tensor | None = None
...
class ActionBlackConditioner(VideoConditioner):
    def forward(
        self,
        batch: dict,
        override_dropout_rate: dict[str, float] | None = None,
    ) -> ActionBlackCondition:
        output = super()._forward(batch, override_dropout_rate)
        assert "action" in batch, "ActionBlackConditioner requires 'action' in batch"
        assert "blacks" in batch, "ActionBlackConditioner requires 'blacks' in batch"
        output["action"] = batch["action"]
        output["blacks"] = batch["blacks"]
        return ActionBlackCondition(**output)

class ActionBlackHistoryConditioner(VideoConditioner):
    def forward(
        self,
        batch: dict,
        override_dropout_rate: dict[str, float] | None = None,
    ) -> ActionBlackHistoryCondition:
        output = super()._forward(batch, override_dropout_rate)
        assert "action" in batch, "ActionBlackHistoryConditioner requires 'action' in batch"
        assert "history" in batch, "ActionBlackHistoryConditioner requires 'history' in batch"
        output["action"] = batch["action"]
        if 'blacks' in batch:
            output["blacks"] = batch["blacks"]
        else:
            output["blacks"] = None
        output["history"] = batch["history"]
        return ActionBlackHistoryCondition(**output)
...
class ConditionLocation(Enum):
    ...
    FIRST_THIRD_VIEW_CAM = "first_third_view_cam"
    MIXED_RANDOM = "mixed_radnom"
...
class MultiViewCondition(VideoCondition):
    ...
    def set_video_condition(
        ...
        elif ConditionLocation.FIRST_THIRD_VIEW_CAM in condition_locations:
            view_indices_B_V_T = self.view_indices_B_T.view(B, sample_n_views, state_t)
            is_third_view_first_B_V = ((view_indices_B_V_T[:, :, 0] == 2) | (view_indices_B_V_T[:, :, 0] == 3))
            m0 = condition_video_input_mask_B_C_V_T_H_W[:, :, :, 0, :, :]
            m0.masked_fill_(is_third_view_first_B_V[:, None, :, None, None], 1)
            condition_video_input_mask_B_C_V_T_H_W[:, :, :, 0, :, :] = m0
        elif ConditionLocation.MIXED_RANDOM in condition_locations:
            view_indices_B_V_T = self.view_indices_B_T.view(B, sample_n_views, state_t)

            # ---- 1) third-view cams (2/3): ALWAYS set first frame mask = 1 ----
            is_third_view_first_B_V = ((view_indices_B_V_T[:, :, 0] == 2) | (view_indices_B_V_T[:, :, 0] == 3))

            m0 = condition_video_input_mask_B_C_V_T_H_W[:, :, :, 0, :, :]  # (B,C,V,H,W)
            m0.masked_fill_(is_third_view_first_B_V[:, None, :, None, None], 1)

            # ---- 2) first-view cams (0/1): SOMETIMES set first frame mask = 1 ----
            is_first_view_first_B_V = ((view_indices_B_V_T[:, :, 0] == 0) | (view_indices_B_V_T[:, :, 0] == 1))

            # mode probs (you can tune):
            #   none: 0.2  (no first-view cam set)
            #   one : 0.3  (choose exactly one first-view cam if multiple)
            #   all : 0.5  (set all first-view cams)
            p_none, p_one, p_all = 0.5, 0.2, 0.3 # ensure p_none + p_one + p_all = 1.0
            device = m0.device
            # rank0 draws randomness, broadcast to all ranks to avoid divergence / hang
            if dist.is_available() and dist.is_initialized():
                r_B = torch.empty(B, device=device)
                r_sel_BV = torch.empty(B, sample_n_views, device=device)
                if dist.get_rank() == 0:
                    r_B.uniform_(0, 1)
                    r_sel_BV.uniform_(0, 1)
                dist.broadcast(r_B, src=0)
                dist.broadcast(r_sel_BV, src=0)
            else:
                r_B = torch.rand(B, device=device)
                r_sel_BV = torch.rand(B, sample_n_views, device=device)

            # decide per-sample mode
            mode_none_B = (r_B < p_none)
            mode_one_B  = (r_B >= p_none) & (r_B < p_none + p_one)
            mode_all_B  = (r_B >= p_none + p_one)

            # if mode_all: set all first-view cams
            add_all_B_V = is_first_view_first_B_V & mode_all_B[:, None]

            # if mode_one: pick one among existing first-view cams
            # trick: mask non-first-view cams to very negative, then argmax on r_sel_BV
            neg_inf = torch.finfo(r_sel_BV.dtype).min
            scores = torch.where(is_first_view_first_B_V, r_sel_BV, torch.full_like(r_sel_BV, neg_inf))
            pick_idx_B = scores.argmax(dim=1)  # (B,)
            add_one_B_V = torch.zeros_like(is_first_view_first_B_V)
            add_one_B_V.scatter_(1, pick_idx_B[:, None], True)  # pick one view per sample
            # if a sample has no first-view cam, scores will be all -inf, argmax=0; we must null it out:
            has_first_B = is_first_view_first_B_V.any(dim=1)
            add_one_B_V = add_one_B_V & has_first_B[:, None] & mode_one_B[:, None]

            # if mode_none: add nothing
            add_first_B_V = add_all_B_V | add_one_B_V

            # finally apply to mask
            m0.masked_fill_(add_first_B_V[:, None, :, None, None], 1)
            condition_video_input_mask_B_C_V_T_H_W[:, :, :, 0, :, :] = m0
...
# add multiview condition to the actionblackhistory condition
@dataclass(frozen=True)
class ActionBlackHistoryMultiviewCondition(MultiViewCondition):
    action: torch.Tensor | None = None
    blacks: torch.Tensor | None = None
    history: torch.Tensor | None = None

class ActionBlackHistoryMultiviewConditioner(GeneralConditioner):
    def forward(
        self,
        batch: dict,
        override_dropout_rate: dict[str, float] | None = None,
    ) -> ActionBlackHistoryMultiviewCondition:
        output = super()._forward(batch, override_dropout_rate)

        assert "action" in batch, "ActionBlackHistoryMultiviewConditioner requires 'action' in batch"
        # assert "history" in batch, "ActionBlackHistoryMultiviewConditioner requires 'history' in batch"
        output["action"] = batch["action"]
        if "blacks" in batch:
            output["blacks"] = batch["blacks"]
        else:
        ...
        return ActionBlackHistoryMultiviewCondition(**output)

@dataclass(frozen=True)
class ActionBlackHistoryStateMultiviewCondition(MultiViewCondition):
    action: torch.Tensor | None = None
    blacks: torch.Tensor | None = None
    history: torch.Tensor | None = None
    state: torch.Tensor | None = None

class ActionBlackHistoryStateMultiviewConditioner(GeneralConditioner):
    def forward(
        self,
        batch: dict,
        override_dropout_rate: dict[str, float] | None = None,
    ) -> ActionBlackHistoryStateMultiviewCondition:
        output = super()._forward(batch, override_dropout_rate)
        ...
        if "state" in batch:
            ...
            output["state"] = None
        return ActionBlackHistoryStateMultiviewCondition(**output)
