...
def _clear_fsdp_post_load_hooks(module: torch.nn.Module):
    for m in module.modules():
        hooks = getattr(m, "_load_state_dict_post_hooks", None)
        if hooks:
            hooks.clear()

def _strip_fsdp_and_clear_hooks(module):
    try:
    ...
    def unwrap(m):
        if FSDP is not None and isinstance(m, FSDP):
            return m.module
        return m

    module = unwrap(module)
    for name, child in list(module.named_children()):
        new_child = unwrap(child)
        if new_child is not child:
            setattr(module, name, new_child)
        _strip_fsdp_and_clear_hooks(new_child)
...
def _sanitize_te_extra_state(d):
    if not isinstance(d, dict):
        return
    for k, v in list(d.items()):
        if isinstance(v, dict):
            _sanitize_te_extra_state(v)
        else:
            if type(v).__name__ == "_EXTRA_STATE":
                print(f"[WARN] Replace TE _EXTRA_STATE at key '{k}' with empty tensor.")
                d[k] = torch.empty(0)
...
class Checkpointer:
    ...
    def load(
        ...
        if self.load_path is not None:
            checkpoint_dir_local_ori = self.checkpoint_dir_local
            self.checkpoint_dir_local = os.path.join(self.load_path, 'checkpoints')
        ...
        if latest_checkpoint_file is not None:
            ...
            if self.load_path is not None:
                self.checkpoint_dir_local = checkpoint_dir_local_ori
        else:
        ...
        if latest_checkpoint_file is not None:
            ...
            state_dicts_paths = {"model": model_checkpoint_path}
            if self.load_training_state:
                state_dicts_paths.update(
                    {
            ...
            for key, checkpoint_path in state_dicts_paths.items():
                ...
                state_dicts_to_load[key] = torch.load(checkpoint_path, map_location=lambda storage, loc: storage, weights_only=False)
            ...
            if is_fsdp:
                ...
                # _sanitize_te_extra_state(state_dicts_to_load_for_dit_reg)
                set_model_state_dict(
                    model.pipe.dit,
                ...
                # Load EMA weights.
                # _sanitize_te_extra_state(state_dicts_to_load_for_dit_ema)
            ...
            torch.cuda.empty_cache()
            if (resume or only_resume_scheduler) and self.load_training_state:
                ...
                iteration = 0
            if resume and self.load_training_state:
                assert optimizer
                log.info("- Loading the optimizer...")
                if is_fsdp:
                    try:
                        set_optimizer_state_dict(
                            model, optimizer, state_dicts_to_load["optim"],
                            options=StateDictOptions(full_state_dict=True, broadcast_from_rank0=True),
                        )
                    except KeyError as e:
                        log.info(f"[ckpt] Optimizer state mismatch: {e}. Skip loading optimizer/scheduler/scaler.")
                else:
    ...
    #         else:
    #             # model.load_state_dict(state_dicts_to_load["model"], strict=self.strict_resume)
    #             _clear_fsdp_post_load_hooks(model.pipe.dit)
    #             iteration = 0
    #             sd_model = state_dicts_to_load["model"]
    #             sd_reg = collections.OrderedDict()
    #             sd_ema = collections.OrderedDict()
    #             for k, v in sd_model.items():
    #                 if k.startswith("net."):
    #                     sd_reg[k[len("net."):]] = v
    #                 elif k.startswith("net_ema."):
    #                     sd_ema[k[len("net_ema."):]] = v
    #                 else:
    #                     sd_reg[k] = v

    #             if hasattr(model, "pipe") and hasattr(model.pipe, "dit"):
    #                 model.pipe.dit.load_state_dict(sd_reg if len(sd_reg) > 0 else sd_model,
    #                                             strict=False, assign=True)
    #                 model.pipe.dit = _strip_fsdp_and_clear_hooks(model.pipe.dit)
    #             else:
    #                 model.load_state_dict(sd_reg if len(sd_reg) > 0 else sd_model,
    #                                     strict=False, assign=True)

    #             if (getattr(getattr(model, "pipe", None), "config", None) is not None
    #                 and getattr(model.pipe.config.ema, "enabled", False)
    #                 and hasattr(model.pipe, "dit_ema")):
    #                 if len(sd_ema) > 0:
    #                     model.pipe.dit_ema.load_state_dict(sd_ema, strict=False, assign=True)
    #                     torch.cuda.empty_cache()
