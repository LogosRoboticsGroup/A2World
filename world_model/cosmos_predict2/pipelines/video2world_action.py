...
class Video2WorldActionConditionedPipeline(Video2WorldPipeline):
    ...
    def __call__(
        ...
        vid_input = torch.from_numpy(first_frame).permute(2, 0, 1)[None, :, None, ...] # (H, W, 3) -> (3, H, W) -> (1, 3, 1, H, W)
        actions_tensor = torch.from_numpy(actions).to(dtype=torch.bfloat16)[None, ...]
        if blacks is not None:
            blacks_tensor = torch.from_numpy(blacks) # need to be (1, 3, T, H, W)
        else:
            blacks_tensor = None
        if history is not None:
        ...
        if 'blacks' in data_batch.keys():
            self._normalize_video_databatch_inplace(data_batch, input_key='blacks', check_key='is_black_preprocessed')
        ...
        if 'history' in data_batch.keys():
            self._normalize_video_databatch_inplace(data_batch, input_key='history', check_key='is_history_preprocessed')
        ...
        x0_fn = self.get_x0_fn_from_batch(data_batch, guidance, is_negative_prompt=True, use_cuda_graphs=use_cuda_graphs)
...
class Video2WorldActionConditionedBatchPipeline(Video2WorldPipeline):
    def __init__(self, device: str = "cuda", torch_dtype: torch.dtype = torch.bfloat16):
        super().__init__(device=device, torch_dtype=torch_dtype)
    ...
    def from_config(
        ...
        # Create a pipe
        pipe = Video2WorldActionConditionedBatchPipeline(device=device, torch_dtype=torch_dtype)
        pipe.config = config
        pipe.precision = {
            "float32": torch.float32,
            "float16": torch.float16,
    ...
    def _get_data_batch_input(
        ...
        blacks_batch: torch.Tensor | None,
        history_batch: torch.Tensor | None,
        prompt: str,
        negative_prompt: str = "",
        num_latent_conditional_frames: int = 1,
    ):
        """
        Prepares the input data batch for the diffusion model. (MODIFIED FOR BATCHING)
        ...
        """
        # MODIFIED: Get batch size dynamically from the input tensor
        batch_size, C, T, H, W = video_batch.shape

        data_batch = {
            "dataset_name": "video_data",
            "video": video_batch, # MODIFIED: Use the passed-in batch
            # MODIFIED: Create dummy embeddings with the correct batch size
            "t5_text_embeddings": torch.zeros(batch_size, 512, 1024, dtype=torch.bfloat16).cuda(),
            "fps": torch.randint(16, 32, (batch_size,)),
            "padding_mask": torch.zeros(batch_size, 1, H, W),
            "num_conditional_frames": num_latent_conditional_frames,
            "action": actions_batch, # MODIFIED: Use the passed-in action batch
        }
        if blacks_batch is not None:
            data_batch['blacks'] = blacks_batch # MODIFIED: Use the passed-in blacks batch
        if history_batch is not None:
            data_batch['history'] = history_batch

        # Handle negative prompts for classifier-free guidance
        if negative_prompt:
            # MODIFIED: Expand negative embeddings to match the batch size
            neg_embeds = self.encode_prompt(negative_prompt).to(dtype=self.torch_dtype)
            data_batch["neg_t5_text_embeddings"] = neg_embeds.repeat(batch_size, 1, 1)
    ...
    def __call__(
        self,
        # MODIFIED: Changed signature to accept lists
        first_frames: List[np.ndarray],
        actions_list: List[np.ndarray],
        blacks_list: List[np.ndarray | None] | None,
        history_list: List[np.ndarray | None] | None,
        ...
        num_sampling_step: int = 35,
        seed: int = 0,
        save_path: str = "output.mp4", # MODIFIED: Added save_path for batch saving
        use_cuda_graphs: bool = False
    ) -> List[torch.Tensor | None]:
        # MODIFIED: Backward compatibility check. If single input, wrap in a list.
        if not isinstance(first_frames, list):
            first_frames = [first_frames]
        if not isinstance(actions_list, list):
            actions_list = [actions_list]
        if blacks_list is None:
            blacks_list = [None] * len(first_frames)
        elif not isinstance(blacks_list, list):
            blacks_list = [blacks_list]
        if history_list is None:
            history_list = [None] * len(first_frames)
        elif not isinstance(history_list, list):
            history_list = [history_list]

        # MODIFIED: Assert that all input lists have the same length
        batch_size = len(first_frames)
        assert len(actions_list) == batch_size, "Number of actions must match number of first frames"
        assert len(blacks_list) == batch_size, "Number of blacks must match number of first frames"
        assert len(history_list) == batch_size, "Number of history must match number of first frames"

        # Parameter check
        assert num_conditional_frames in [1, 5], "num_conditional_frames must be 1 or 5"
        num_latent_conditional_frames = self.tokenizer.get_latent_num_frames(num_conditional_frames)

        # MODIFIED: Loop to process and collect inputs into a batch
        vid_inputs_list = []
        actions_tensors_list = []
        blacks_tensors_list = []
        history_tensors_list = []
        has_blacks = any(b is not None for b in blacks_list)
        has_history = any(b is not None for b in history_list)


        for i in range(batch_size):
            # transform first frame and actions to tensor
            vid_input = torch.from_numpy(first_frames[i]).permute(2, 0, 1)[None, :, None, ...]
            actions_tensor = torch.from_numpy(actions_list[i]).to(dtype=torch.bfloat16)[None, ...]
            vid_inputs_list.append(vid_input)
            actions_tensors_list.append(actions_tensor)

            if has_blacks:
                if blacks_list[i] is not None:
                    blacks_tensors_list.append(torch.from_numpy(blacks_list[i]))
                else:
                    # If some have blacks and others don't, we need to add a dummy tensor
                    # This part might need adjustment based on how 'blacks' is used
                    dummy_shape = actions_list[i].shape[:-1] + (vid_input.shape[-2], vid_input.shape[-1])
                    blacks_tensors_list.append(torch.zeros(dummy_shape, dtype=torch.bool))

            if has_history:
                if history_list[i] is not None:
                    history_tensors_list.append(torch.from_numpy(history_list[i]))
                else:
                    # If some have history and others don't, we need to add a dummy tensor
                    # This part might need adjustment based on how 'history' is used
                    dummy_shape = actions_list[i].shape[:-1] + (vid_input.shape[-2], vid_input.shape[-1])
                    history_tensors_list.append(torch.zeros(dummy_shape, dtype=torch.bool))

        # MODIFIED: Concatenate lists of tensors into a single batch tensor
        vid_input_batch = torch.cat(vid_inputs_list, dim=0)
        actions_tensor_batch = torch.cat(actions_tensors_list, dim=0)
        blacks_tensor_batch = torch.stack(blacks_tensors_list, dim=0) if has_blacks else None
        history_tensor_batch = torch.stack(history_tensors_list, dim=0) if has_history else None
        ...
        # --- The rest of the pipeline logic is mostly batch-aware already ---
        # preprocess
        self._normalize_video_databatch_inplace(data_batch)
        self._augment_image_dim_inplace(data_batch)
        if 'blacks' in data_batch.keys():
        ...
        # MODIFIED: n_sample is now our dynamic batch_size
        n_sample = data_batch[input_key].shape[0]
        _T, _H, _W = data_batch[input_key].shape[-3:]
        state_shape = [
            self.config.state_ch,
        ...
        x0_fn = self.get_x0_fn_from_batch(data_batch, guidance, is_negative_prompt=True, use_cuda_graphs=use_cuda_graphs)
        log.info(f"Starting video generation for a batch of {n_sample}...")
        x_sigma_max = (
            misc.arch_invariant_rand(
        ...
        # --- SAMPLING LOOP (no changes needed, it works on the whole batch `sample`) ---
        scheduler = self.scheduler
        scheduler.set_timesteps(num_sampling_step, device=x_sigma_max.device)
        sample = x_sigma_max.to(dtype=torch.float32)
        x0_prev: torch.Tensor | None = None
        for i, _ in enumerate(scheduler.timesteps):
            ...
            sigma_in = sigma_t.repeat(sample.shape[0])
            x0_pred = x0_fn(sample, sigma_in)
            sample, x0_prev = scheduler.step(x0_pred=x0_pred, i=i, sample=sample, x0_prev=x0_prev)
        sigma_min = scheduler.sigmas[-1].to(sample.device, dtype=torch.float32)
        sigma_in = sigma_min.repeat(sample.shape[0])
        samples = x0_fn(sample, sigma_in)
        ...
        # --- POST-PROCESSING & SAVING (MODIFIED FOR BATCHING) ---
        # Decode the entire batch at once
        video_batch = self.decode(samples)  # shape: (B, C, T, H, W)

        output_videos = []
        base, ext = os.path.splitext(save_path) # For generating unique filenames

        for i in range(batch_size):
            video = video_batch[i:i+1] # Keep batch dim for compatibility
            ...
            if self.video_guardrail_runner is not None:
                ...
                if processed_frames is None:
                    log.warning(f"Video {i} in batch failed guardrail check.")
                    output_videos.append(None)
                    continue # Skip to the next video in the batch
                else:
                    log.success(f"Video {i} passed guardrail on generated video")
                    processed_video = torch.from_numpy(processed_frames).float().permute(3, 0, 1, 2) / 255.0
                    processed_video = processed_video * 2 - 1
                    processed_video = processed_video.unsqueeze(0)
                    video = processed_video.to(video.device, dtype=video.dtype)

            # MODIFIED: Save the individual video with a unique name
            # current_save_path = f"{base}_{i}{ext}"
            # Assuming you have a function to save the tensor to a video file
            # You might need to implement or find this function, e.g., save_video_from_tensor(video, current_save_path)
            # log.info(f"Saving video {i} to {current_save_path}")

            output_videos.append(video)

        log.success(f"Batch video generation completed successfully for {len(output_videos)} videos.")
