...
def read_and_process_multiview_video(
    ...
    if video_tensor is not None:
        ...
        video_v = video_tensor.clone().detach().to(torch.uint8)
        available_frames_per_view = video_v.shape[1] // n_views
        video_v = video_v.view(C, n_views, available_frames_per_view, H, W)
        video_v[:, :, 1:, :, :] = 0
        video_v = video_v.view(C, T, H, W)
        return video_v.unsqueeze(0)
