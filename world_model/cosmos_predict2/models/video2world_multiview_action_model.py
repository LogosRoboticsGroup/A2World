...
class Predict2Video2WorldActionConditionedMultiviewModel(Predict2Video2WorldModel):
    def __init__(self, config: Predict2ActionConditionedMultiviewModelConfig):
        ...
        self.pipe = MultiviewVideo2WorldActionConditionedPipeline.from_config(
            config.pipe_config,
    ...
    def validation_step(self, data_batch: dict, data_batch_idx: int) -> tuple[dict, torch.Tensor]:
        video_tensor = data_batch['video'][0].detach().clone()
        ...
        if 'history' in data_batch.keys():
            ...
            input_history = None
        video = self.pipe(video_tensor, None, actions=input_action, blacks=input_blacks, history=input_history, latent_view_indices=latent_view_indices_B_T, num_conditional_frames=1, guidance=0, n_views=n_views, use_cuda_graphs=False)
