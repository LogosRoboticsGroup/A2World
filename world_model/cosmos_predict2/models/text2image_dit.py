...
class Attention(nn.Module):
    ...
    def forward(
        ...
        k_mem: torch.Tensor | None = None,   # [B, Lm, K]，K=query_dim
        v_mem: torch.Tensor | None = None,   # [B, Lm, K]
    ) -> torch.Tensor:
        """
        Args:
        ...
        if (k_mem is not None) and (v_mem is not None):
            if len(k_mem.shape) == 3 and len(v_mem.shape) == 3:
                # single-view generation
                k_m = self.k_proj(k_mem)  # [B, Lm, H*Dh]
                v_m = self.v_proj(v_mem)  # [B, Lm, H*Dh]
                k_m = rearrange(k_m, "b s (h d) -> b s h d", h=self.n_heads, d=self.head_dim)
                v_m = rearrange(v_m, "b s (h d) -> b s h d", h=self.n_heads, d=self.head_dim)
                k_m = self.k_norm(k_m); v_m = self.v_norm(v_m)
                k = torch.cat([k_m, k], dim=1)   # [B, Lm+S, H, Dh]
                v = torch.cat([v_m, v], dim=1)
            elif len(k_mem.shape) == 4 and len(v_mem.shape) == 4:
                # multi-view generation
                n_views = k_mem.shape[1]
                k_m = self.k_proj(k_mem)  # [B, V, Lm, H*Dh]
                v_m = self.v_proj(v_mem)  # [B, V, Lm, H*Dh]
                k_m = rearrange(k_m, "b v s (h d) -> b v s h d", h=self.n_heads, d=self.head_dim)
                v_m = rearrange(v_m, "b v s (h d) -> b v s h d", h=self.n_heads, d=self.head_dim)
                k_m = self.k_norm(k_m); v_m = self.v_norm(v_m)
                k = rearrange(k, "b (n t) h d -> b n t h d", n=n_views)
                v = rearrange(v, "b (n t) h d -> b n t h d", n=n_views)
                k = torch.cat([k_m, k], dim=2).flatten(1, 2)   # [B, V, Lm+S, H, Dh]
                v = torch.cat([v_m, v], dim=2).flatten(1, 2)
            else:
                raise("Unsupported history latent!")
...
class MiniTrainDIT(WeightTrainingStat):
    ...
    def prepare_embedded_sequence(
        ...
        x_B_T_H_W_D = self.x_embedder(x_B_C_T_H_W) #(B, ld+2, lt, lh, lw) -> (B, lt, lh/2, lw/2, 2048)
